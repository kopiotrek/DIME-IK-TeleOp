...
class TeleOp(object):
    ...
    # def __init__(self, record_demo=False, hide_window=False, cfg=None, enable_moving_average=True):
        ...
        print(f"{finger_type} controller starting...")

        try:
            rospy.init_node(f'hardware_teleop_{finger_type}')
        except rospy.ROSException as e:
            print(f'Node initialization failed: {finger_type+str(e)}')
            pass
        ...
        print(f"{finger_type} controller initialized!")
    ...
    def hand_pose(self, action=np.zeros(16)):
        ...
        if self.finger_type == 'index':
            action[:3] = action[:3]  # Use the provided action values
            action[4:] = current_angles[4:]  # Retain the current joint angles

        elif self.finger_type == 'middle':
            action[:3] = current_angles[:3]  # Retain the current joint angles
            action[4:7] = action[4:7]  # Use the provided action values
            action[8:] = current_angles[8:]  # Retain the current joint angles

        elif self.finger_type == 'ring':
            action[:7] = current_angles[:7]  # Retain the current joint angles
            action[8:11] = action[8:11]  # Use the provided action values
            action[12:] = current_angles[12:]  # Retain the current joint angles

        elif self.finger_type == 'thumb':
            action[:11] = current_angles[:11]  # Retain the current joint angles
            action[12:] = action[12:]  # Use the provided action values
...
def control_finger(finger_type):
    teleop = TeleOp(finger_type)
    teleop.control_finger()
    while True:
        continue
